...
var (
	...
	paw              = ""
	group            = "rce" // Default to RCE group
	c2Name           = "HTTP"
	c2Key            = ""
	...
	rcePlatform      = ""
	rceExecutor      = ""
	rceCommand       = "COMMAND" // Default to substitution marker
	rcePayloadName   = ""
)
...
func main() {
	...
	rcePlatform := flag.String("rcePlatform", rceExecutor, "Platform to use for remotely executed commands")
	rceExecutor := flag.String("rceExecutor", rceExecutor, "Executor to use for remotely executed commands")
	rceCommand := flag.String("rceCommand", rceCommand, "Base command to use for substituting incoming commands (use COMMAND as position for substitution)")
	rcePayloadName := flag.String("rcePayloadName", rcePayloadName, "Optionally pass payload file that is necessary for executing the RCE command")
	...
	if err != nil && *verbose {
		fmt.Printf("[!] Error building tunnel config: %s", err.Error())
		return
	}
	...
	core.Core(trimmedServer, tunnelConfig, *group, *delay, contactConfig, *listenP2P, *verbose, *paw, *originLinkID, *rceCommand, *rceExecutor, *rcePlatform, *rcePayloadName)
}
